...
class CloudKitManager: ObservableObject {
    ...
    func fetchUserProfile(byUserID userID: String) async throws -> CKRecord? {
        ...
        AppLogger.cloudKit.debug("fetchUserProfile byUserID: \(userID)")
        do {
            let results = try await fetchFromPublic(recordType: "UserProfile", predicate: predicate, limit: 1)
            AppLogger.cloudKit.debug("fetchUserProfile returned \(results.count) records")
            return results.first
        } catch let error as CKError where error.code == .unknownItem {
            AppLogger.cloudKit.debug("fetchUserProfile: schema doesn't exist")
            return nil
        } catch {
            AppLogger.cloudKit.error("fetchUserProfile ERROR: \(error.localizedDescription)")
            throw error
        }
    }
    ...
    func fetchFriendConnections(forUserID userID: String) async throws -> [CKRecord] {
        ...
        AppLogger.cloudKit.debug("fetchFriendConnections for userID: \(userID)")
        ...
        AppLogger.cloudKit.debug("fetchFriendConnections query 1: \(predicate1)")
        do {
            let records1 = try await fetchFromPublic(recordType: "FriendConnection", predicate: predicate1)
            AppLogger.cloudKit.debug("fetchFriendConnections query 1 returned \(records1.count) records")
            ...
            AppLogger.cloudKit.debug("fetchFriendConnections query 1: schema doesn't exist yet")
        } catch {
            AppLogger.cloudKit.error("fetchFriendConnections query 1 ERROR: \(error.localizedDescription)")
        ...
        AppLogger.cloudKit.debug("fetchFriendConnections query 2: \(predicate2)")
        do {
            let records2 = try await fetchFromPublic(recordType: "FriendConnection", predicate: predicate2)
            AppLogger.cloudKit.debug("fetchFriendConnections query 2 returned \(records2.count) records")
            ...
            AppLogger.cloudKit.debug("fetchFriendConnections query 2: schema doesn't exist yet")
        } catch {
            AppLogger.cloudKit.error("fetchFriendConnections query 2 ERROR: \(error.localizedDescription)")
        }

        AppLogger.cloudKit.debug("fetchFriendConnections total: \(allRecords.count) records")
        return allRecords
    }
    ...
    func fetchPendingRequests(forUserID userID: String) async throws -> [CKRecord] {
        ...
        AppLogger.cloudKit.debug("fetchPendingRequests for targetID: \(userID)")
        AppLogger.cloudKit.debug("fetchPendingRequests predicate: \(predicate)")
        do {
            let records = try await fetchFromPublic(recordType: "FriendConnection", predicate: predicate)
            AppLogger.cloudKit.debug("fetchPendingRequests returned \(records.count) records")
            for record in records {
                ...
                AppLogger.cloudKit.debug("  - Record: requesterID=\(requesterID), targetID=\(targetID), status=\(status)")
            }
            return records
        } catch let error as CKError where error.code == .unknownItem {
            AppLogger.cloudKit.debug("fetchPendingRequests: schema doesn't exist yet")
            return []
        } catch {
            AppLogger.cloudKit.error("fetchPendingRequests ERROR: \(error.localizedDescription)")
            throw error
        }
    }
    ...
    func fetchSentRequests(forUserID userID: String) async throws -> [CKRecord] {
        ...
        AppLogger.cloudKit.debug("fetchSentRequests for requesterID: \(userID)")
        do {
            let records = try await fetchFromPublic(recordType: "FriendConnection", predicate: predicate)
            AppLogger.cloudKit.debug("fetchSentRequests returned \(records.count) records")
            return records
        } catch let error as CKError where error.code == .unknownItem {
            AppLogger.cloudKit.debug("fetchSentRequests: schema doesn't exist yet")
            return []
        } catch {
            AppLogger.cloudKit.error("fetchSentRequests ERROR: \(error.localizedDescription)")
            throw error
        }
    }
    ...
    func createSubscription(
        ...
        guard isAvailable else {
            AppLogger.cloudKit.info("Skipping subscription \(subscriptionID): CloudKit unavailable")
            return false
        }
        ...
        do {
            ...
            AppLogger.cloudKit.info("Created subscription: \(subscriptionID)")
            return true
            ...
            do {
                ...
                try await publicDatabase.save(subscription)
                AppLogger.cloudKit.info("Updated subscription: \(subscriptionID)")
                return true
            } catch {
    ...
    func removeSubscription(subscriptionID: String) async {
        do {
            try await publicDatabase.deleteSubscription(withID: subscriptionID)
            AppLogger.cloudKit.info("Removed subscription: \(subscriptionID)")
        } catch let error as CKError where error.code == .unknownItem {
            // Subscription doesn't exist, that's fine
            AppLogger.cloudKit.debug("Subscription \(subscriptionID) not found (already removed)")
        } catch {
    ...
    func fetchAllSubscriptions() async -> [CKSubscription] {
        do {
            ...
            AppLogger.cloudKit.error("Failed to fetch subscriptions: \(error.localizedDescription)")
            return []
        }
    }
}
